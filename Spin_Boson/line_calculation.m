function out = line_calculation(line)
    ...
    A(t) = x * (cos(2* alpha(t)) + 2* (n(1) * sin(alpha(t)))^2 ) - y * n(3) * sin(2 * alpha(t)) + z *2 * n(1) * n(3) * (sin(alpha(t)))^2;
    %Now on spin boson
    H_0 = (e/2) .* z + (del / 2) .* x;
    ...
    %     line = [line(1:max(1, first_int(1) -1)) , result , line(min(final_int(1) + 1, strlength(line)) : end)
        if endIn(k) < length(line)
            %the first hadmard term
                ...
                hadout = [hadout, result, line(min(endIn(k) + 1, length(line)) : end)];
        end
    end
